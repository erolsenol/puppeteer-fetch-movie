...
async function StartFilmCehennemi(nextPage) {
  if (startIndex < 1) {
...
async function pageMovieUrlsSet() {
  return new Promise(async (resolve, reject) => {
    const movieUrls = [];
    await page.waitForSelector("div[id='listehizala']");
    const movieElements = await page.$$(
      "div[id='listehizala'] > div[class='listmovie']"
    );
    if (!movieElements) {
    ...
    for (let index = 0; index < movieElements.length; index++) {
      const movieUrl = await movieElements[index].evaluate((item) => {
        if (item.children[0].children.length == 2) {
          return item.children[0].children[0].children[0].getAttribute("href");
        } else if (item.children[0].children.length == 3) {
          return item.children[0].children[1].children[0].getAttribute("href");
        } else if (item.children[0].children.length == 4) {
          return item.children[0].children[2].children[0].getAttribute("href");
        }
      });
...
async function nextPageSet() {
  return new Promise(async (resolve, reject) => {
    ...
    const nextBtnArea = await page.$("div[class='sayfalama']");

    if (nextBtnArea) {
      const nextPageUrl = await nextBtnArea.evaluate((area) => {
        if (
          area.children[area.children.length - 1].getAttribute("class") != "on"
        ) {
          let activeIsFind = false,
            findUrl;
          for (const el of area.children) {
            if (activeIsFind) {
              findUrl = el.getAttribute("href");
              break;
            }
            if (el.getAttribute("class") == "on") {
              activeIsFind = true;
            }
          }
          if (findUrl) {
